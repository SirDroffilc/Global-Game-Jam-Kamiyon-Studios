...
func start_all_spawners() -> void:
	# Run each spawner loop independently
	run_spawn_loop(enemy_1_scene, interval_fast, enemy_marker_1, "Enemy 1")
	run_spawn_loop(enemy_2_scene, interval_fast, enemy_marker_2, "Enemy 2")
	run_spawn_loop(enemy_3_scene, interval_medium, enemy_marker_3, "Enemy 3")
	run_spawn_loop(enemy_4_scene, interval_slow, enemy_marker_4, "Enemy 4")

func run_spawn_loop(enemy_scene: PackedScene, interval: float, marker: Marker2D, debug_name: String) -> void:
	var my_session = current_spawn_session
	
	print(">>> SPAWNER: Starting loop for ", debug_name, " in Session ", my_session)
	
	for i in range(spawn_count):
		# FIRST: Wait for the interval. 
		# This ensures Enemy 4 waits 120s before the very first spawn.
